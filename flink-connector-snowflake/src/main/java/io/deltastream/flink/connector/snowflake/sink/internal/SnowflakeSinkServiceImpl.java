...
public class SnowflakeSinkServiceImpl implements SnowflakeSinkService {
    ...
    // TODO in favor of configurable value, make this default and lower the threshold
    ...
    public SnowflakeSinkServiceImpl(
        ...
        // ingest client
        this.client = this.createClientFromConfig(appId, channelConfig, connectionConfig);
    ...
    /**
     * Close the current channel and open a new one to reset the internal state of the channel,
     * which is recommended by the Snowflake service after a certain threshold of inserted rows to
    ...
    void recreateChannel() {
        ...
        try {
            ...
            /*
             * If we end up here, there may be rows that didn't get committed to Snowflake due to the closeChannel().
             * We also failed to open a new channel to continue processing, which means the sink cannot make any more
             * progress. Therefore, we treat this as a fatal error
             */
            throw new FlinkRuntimeException(
                    String.format(
                            "Failed to recreate Snowflake ingest channel '%s'", this.channelName),
                    e);
        }
    }
